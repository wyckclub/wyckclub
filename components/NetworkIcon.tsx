...
export function NetworkIcon({ chain, className }: { chain: 'base' | 'robinhood'; className?: string }) {
  return chain === 'robinhood' ? (
    <RobinhoodNetworkIcon className={className} />
  ) : (
    <BaseNetworkIcon className={className} />
  );
}
